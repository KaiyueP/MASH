...
N_QD = nqd
g_s_ref_eV=20e-3 #eV to au #coupling strength
...
# ============================================================
# Transform from exciton/cavity basis to polaritonic basis
# ============================================================
# np.linalg.eigh returns eigenvectors as columns:
#   |polariton a> = sum_i U[i, a] |exciton/cavity i>
# Therefore operators in the polariton basis are U.conj().T @ A @ U.
ham_sys_exciton_AU = ham_sys_AU.copy()
Vklq_exciton_AU = Vklq_AU.copy()
polariton_energies_AU, exciton_to_polariton_U = np.linalg.eigh(ham_sys_exciton_AU)

ham_sys_AU = np.diag(polariton_energies_AU)
Vklq_AU = np.einsum("ia,kij,jb->kab", exciton_to_polariton_U.conj(), Vklq_exciton_AU, exciton_to_polariton_U)
exciton_state_labels = np.array(
	[f"QD1_{i}" for i in range(nstate_per_qd)]
	+ [f"QD2_{i}" for i in range(nstate_per_qd)]
	+ ["cavity"]
)
...
np.savez(
	...
	polariton_energies_AU=polariton_energies_AU,
